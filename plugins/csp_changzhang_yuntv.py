...
class Spider(_BaseSpider):
    ...
    def init(self, extend=""):
        """兼容云TV盒子的字符串 / JSON / 字典形式 extend 参数。"""
    ...
    # ==================== 筛选数据 ====================
    # 站点三套 taxonomy。路径取自首页导航 + sitemap, 已校对。
    # 注意: 该站不支持多维度组合筛选, 每个 taxonomy 项都是一条独立列表路径。
    TAGS = [
    ...
    # 剧集片库: 修正了欧美电影 slug(meiguodianying→oumeidianying), 补充海外剧
    SERIES = [
    ...
    def homeVideoContent(self):
        return {'list': self.categoryContent('movie_bt', 1, False, {}).get('list', [])}
    ...
    @staticmethod
    def _first_id(ids):
        """兼容云TV盒子桥接后的列表，以及少数实现直接传入 JSON 字符串的情况。"""
        value = ids
    ...
    def searchContent(self, key, quick=False, pg="1"):
        ...
        key = str(key).strip()
        # 搜索路径取自首页表单 action="/nimasile", name="q"
        url = '%s/nimasile?q=%s' % (self._site(), urllib.parse.quote(key))
        vod_list = self._parse_list(self._get(url))

        # 若新路径无结果, 回退旧路径
        if not vod_list:
            url2 = '%s/boss1O1?q=%s' % (self._site(), urllib.parse.quote(key))
            vod_list = self._parse_list(self._get(url2))
